...
class Database:
    ...
    def restore_from_file(self, file_path: Union[str, Path]) -> dict:
        ...
        try:
            ...
            if suffix == ".json":
                ...
                tables_data = backup_data.get("tables", {})
                if not tables_data:
                    return {"success": False, "error": "فایل JSON شامل ساختار معتبر جداول نمی‌باشد."}
        ...
        try:
            ...
            if old_status in ("approved", "completed"):
                ...
                if r_id and is_bundle:
                    ...
                    # ابطال پرداخت مشتری نماینده توسط مدیر: استرداد هزینه خرید عمده به کیف پول یا کاهش بدهی اعتباری
                    r_tx = cursor.execute("""
                        SELECT id, amount, payment_source, type FROM reseller_transactions 
                    ...
                    if r_tx:
                        ...
                        is_credit = (r_tx["payment_source"] == "credit" or r_tx["type"] in ("purchase_credit", "renewal_credit"))
                        r_cur = cursor.execute("SELECT balance FROM resellers WHERE id=?", (r_id,)).fetchone()
                        r_cur_bal = int(r_cur[0] or 0) if r_cur else 0
                        if is_credit:
                            cursor.execute("UPDATE resellers SET credit_debt = MAX(0, credit_debt - ?), updated_at=? WHERE id=?", (ref_amount, now_iso, r_id))
                            cursor.execute("""
                                INSERT INTO reseller_transactions (reseller_id, type, amount, balance_after, plan_name, description, payment_source, created_by, created_at)
                                VALUES (?, 'refund_credit', ?, ?, ?, ?, 'credit', ?, ?)
                            """, (r_id, ref_amount, r_cur_bal, tx.get("plan_name"), f"کاهش بدهی اعتباری ناشی از ابطال سفارش #{tx['order_id']} توسط مدیریت ({admin_name})", admin_name, now_iso))
                        else:
                            new_r_bal = r_cur_bal + ref_amount
                            cursor.execute("UPDATE resellers SET balance = ?, updated_at=? WHERE id=?", (new_r_bal, now_iso, r_id))
                            cursor.execute("""
                                INSERT INTO reseller_transactions (reseller_id, type, amount, balance_after, plan_name, description, payment_source, created_by, created_at)
                                VALUES (?, 'refund', ?, ?, ?, ?, 'wallet', ?, ?)
                            """, (r_id, ref_amount, new_r_bal, tx.get("plan_name"), f"استرداد وجه ناشی از ابطال سفارش #{tx['order_id']} توسط مدیریت ({admin_name})", admin_name, now_iso))
        ...
        try:
            ...
            if old_status in ("approved", "completed"):
                ...
                if refund_wallet and refund_amount > 0:
                    is_credit = bool(r_tx and (r_tx.get("payment_source") == "credit" or r_tx.get("type") in ("purchase_credit", "renewal_credit")))
                    r_cur = cursor.execute("SELECT balance FROM resellers WHERE id=?", (reseller_id,)).fetchone()
                    r_cur_bal = int(r_cur[0] or 0) if r_cur else 0
                    if is_credit:
                        cursor.execute("UPDATE resellers SET credit_debt = MAX(0, credit_debt - ?), updated_at=? WHERE id=?", (refund_amount, now_iso, reseller_id))
                        cursor.execute("""
                            INSERT INTO reseller_transactions (reseller_id, type, amount, balance_after, plan_name, account_name, description, payment_source, created_by, created_at)
                            VALUES (?, 'refund_credit', ?, ?, ?, ?, ?, 'credit', ?, ?)
                        """, (
                            reseller_id,
                            refund_amount,
                            ...
                            tx.get("plan_name"),
                            account_name,
                            f"کاهش بدهی اعتباری ناشی از ابطال فیش #{tx_id} (سفارش {order_id}) - علت: {reason}",
                            reseller_name,
                            now_iso
                        ))
                    else:
                        new_r_bal = r_cur_bal + refund_amount
                        cursor.execute("UPDATE resellers SET balance = ?, updated_at=? WHERE id=?", (new_r_bal, now_iso, reseller_id))
                        cursor.execute("""
                            INSERT INTO reseller_transactions (reseller_id, type, amount, balance_after, plan_name, account_name, description, payment_source, created_by, created_at)
                            VALUES (?, 'refund', ?, ?, ?, ?, ?, 'wallet', ?, ?)
                        """, (
    ...
    def reseller_soft_delete_transaction(self, tx_id: int, reseller_id: int, reseller_name: str, reason: str = "") -> dict:
        ...
        try:
            ...
            # در صورتی که فیش قبلاً تایید شده و باطل نشده، اثرات مالی آن رول‌بک می‌شود
            if tx.get("status") in ("approved", "completed"):
                order_id = tx.get("order_id") or ""
                account_name = tx.get("account_name") or ""
                r_tx = None
                if account_name:
                ...
                if r_tx:
                    refund_amount = int(r_tx["amount"] or 0)
                    is_credit = bool(r_tx.get("payment_source") == "credit" or r_tx.get("type") in ("purchase_credit", "renewal_credit"))
                    if is_credit:
                        cursor.execute("UPDATE resellers SET credit_debt = MAX(0, credit_debt - ?), updated_at=? WHERE id=?", (refund_amount, now_iso, reseller_id))
                    else:
                    ...
                    cursor.execute("""
                        UPDATE reseller_transactions
                        SET is_deleted=1, status='revoked', is_revoked=1, revoked_by=?, revoked_at=?, revoke_reason=?
                        WHERE id=?
                    """, (reseller_name, now_iso, f"حذف فیش توسط نماینده: {reason}", r_tx["id"]))

                # رول‌بک تراکنش کارت بانکی نماینده
                try:
                    c_tx = cursor.execute("""
                        SELECT id, card_id, amount FROM card_transactions
                        WHERE owner_type='reseller' AND reseller_id=? AND ref_type='transaction' AND ref_id=?
                    ...
                    """, (reseller_id, str(tx_id))).fetchone()
                    if c_tx:
                        cursor.execute("UPDATE card_transactions SET is_revoked=1, revoke_reason='حذف فیش توسط نماینده' WHERE id=?", (c_tx["id"],))
                        cursor.execute("UPDATE reseller_cards SET balance = balance - ? WHERE id=?", (c_tx["amount"], c_tx["card_id"]))
                except Exception:
                    pass
            ...
            """, (tx_id, reseller_id, reseller_name, reason or "حذف فیش توسط نماینده", now_iso))
    ...
    def admin_delete_reseller_receipt(
        self,
        tx_id: int,
        admin_id: int,
        admin_name: str,
        reason: str = "",
        rollback_finances: bool = True
    ) -> dict:
        """
        حذف فیش ثبت‌شده نماینده توسط مدیریت به همراه رول‌بک و تعدیل کامل موجودی و بدهی
        """
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            ...
            if rollback_finances and tx.get("status") in ("approved", "completed") and not tx.get("is_revoked"):
                is_bundle = (tx.get("gateway") == "bundle_reseller" or str(tx.get("order_id") or "").startswith("R_BUNDLE"))
                if r_id and is_bundle:
                    # کسر موجودی شارژشده بسته تستی از نماینده
                    amount = int(tx.get("amount") or 0)
                    cursor.execute("UPDATE resellers SET balance = MAX(0, balance - ?), updated_at=? WHERE id=?", (amount, now_iso, r_id))
                    cursor.execute("""
                        UPDATE reseller_transactions
                        SET is_deleted=1, status='revoked', is_revoked=1, revoked_by=?, revoked_at=?, revoke_reason=?
                        WHERE reseller_id=? AND (description LIKE ? OR plan_name=?) AND type='deposit'
                    """, (admin_name, now_iso, reason or "حذف فیش بسته توسط مدیریت", r_id, f"%{tx['order_id']}%", tx.get("plan_name")))
                elif r_id and not is_bundle:
                    # خرید مشتری روی پنل نماینده: بازگشت اعتبار/وجه
                    r_tx = cursor.execute("""
                        SELECT id, amount, payment_source, type FROM reseller_transactions 
                        WHERE reseller_id=? AND (description LIKE ? OR plan_name=?) AND type IN ('purchase', 'purchase_credit', 'renewal')
                          AND (is_revoked=0 OR is_revoked IS NULL)
                    ...
                    if r_tx:
                        ...
                        cursor.execute("""
                            ...
                            SET is_deleted=1, status='revoked', is_revoked=1, revoked_by=?, revoked_at=?, revoke_reason=?
                            WHERE id=?
                        """, (admin_name, now_iso, reason or "حذف فیش توسط مدیریت", r_tx["id"]))

                # رول‌بک کارت بانکی در صورت وجود
                try:
                    c_tx = cursor.execute("""
                        SELECT id, card_id, amount FROM card_transactions
                        WHERE ref_type='transaction' AND ref_id=? AND (is_revoked=0 OR is_revoked IS NULL)
                        ORDER BY id DESC LIMIT 1
                    """, (str(tx_id),)).fetchone()
                    if c_tx:
                        cursor.execute("UPDATE card_transactions SET is_revoked=1, revoke_reason=? WHERE id=?", (f"حذف فیش توسط {admin_name}", c_tx["id"]))
                        cursor.execute("UPDATE reseller_cards SET balance = MAX(0, balance - ?) WHERE id=?", (c_tx["amount"], c_tx["card_id"]))
                except Exception:
                    pass

            # علامت‌گذاری حذف فیش
            cursor.execute("UPDATE transactions SET is_deleted=1, status='deleted', updated_at=? WHERE id=?", (now_iso, tx_id))
            cursor.execute("""
                INSERT INTO transaction_audit_logs 
                (transaction_id, admin_id, admin_name, action, field_name, old_value, new_value, reason, created_at)
                VALUES (?, ?, ?, 'delete', 'is_deleted', '0', '1', ?, ?)
            """, (tx_id, admin_id, admin_name, reason or "حذف فیش توسط مدیریت با تعدیل مالی", now_iso))

            conn.commit()
            return {"success": True, "reseller_id": r_id}
        except Exception as e:
            conn.rollback()
            return {"success": False, "error": str(e)}
        finally:
    ...
    def admin_delete_reseller_wallet_tx(
        self,
        rtx_id: int,
        admin_id: int,
        admin_name: str,
        ...
        """
        حذف تراکنش کیف پول نماینده توسط مدیریت با تعدیل خودکار موجودی یا بدهی اعتباری
        """
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            r_tx = cursor.execute("SELECT * FROM reseller_transactions WHERE id=?", (rtx_id,)).fetchone()
            if not r_tx:
                return {"success": False, "error": "تراکنش کیف پول یافت نشد."}
            r_tx = dict(r_tx)
            r_id = r_tx.get("reseller_id")
            amount = int(r_tx.get("amount") or 0)
            ttype = r_tx.get("type")
            is_revoked = bool(r_tx.get("is_revoked"))
            now_iso = get_now_iso()

            if rollback_finances and not is_revoked and amount > 0:
                if ttype == "deposit":
                    # واریز تستی به کیف پول: کسر از موجودی نماینده
                    cursor.execute("UPDATE resellers SET balance = MAX(0, balance - ?), updated_at=? WHERE id=?", (amount, now_iso, r_id))
                elif ttype in ("purchase_credit", "renewal_credit") or r_tx.get("payment_source") == "credit":
                    # خرید با اعتبار: کاهش بدهی نماینده
                    cursor.execute("UPDATE resellers SET credit_debt = MAX(0, credit_debt - ?), updated_at=? WHERE id=?", (amount, now_iso, r_id))
                elif ttype in ("purchase", "renewal"):
                    # خرید از کیف پول: استرداد وجه به کیف پول نماینده
                    cursor.execute("UPDATE resellers SET balance = balance + ?, updated_at=? WHERE id=?", (amount, now_iso, r_id))

            # علامت‌گذاری به عنوان حذف شده
            cursor.execute("UPDATE reseller_transactions SET is_deleted=1, status='deleted' WHERE id=?", (rtx_id,))
            cursor.execute("""
                INSERT INTO reseller_transaction_audit_logs
                (reseller_transaction_id, admin_id, admin_name, action, field_name, old_value, new_value, reason, created_at)
                VALUES (?, ?, ?, 'delete', 'is_deleted', '0', '1', ?, ?)
            """, (rtx_id, admin_id, admin_name, reason or "حذف تراکنش کیف پول توسط مدیریت با تعدیل حساب", now_iso))
    ...
    def admin_reset_reseller_finances(
        self,
        reseller_id: int,
        admin_id: int,
        admin_name: str,
        reset_balance: bool = True,
        reset_credit_debt: bool = True,
        purge_receipts: bool = True,
        purge_wallet_txs: bool = True,
        ...
        cursor = conn.cursor()
        try:
            r = cursor.execute("SELECT * FROM resellers WHERE id=?", (reseller_id,)).fetchone()
            if not r:
            ...
            # ۱. صفر کردن موجودی کیف پول نقدی
            if reset_balance:
                cursor.execute("UPDATE resellers SET balance = 0, updated_at=? WHERE id=?", (now_iso, reseller_id))

            # ۲. صفر کردن کل بدهی اعتباری
            if reset_credit_debt:
                cursor.execute("UPDATE resellers SET credit_debt = 0, updated_at=? WHERE id=?", (now_iso, reseller_id))
                try:
                    cursor.execute("""
            ...
            # ۳. پاکسازی و حذف فیش‌ها و رسیدهای ثبت‌شده نماینده
            if purge_receipts:
                cursor.execute("UPDATE transactions SET is_deleted=1, updated_at=? WHERE reseller_id=?", (now_iso, reseller_id))

            # ۴. پاکسازی و حذف تراکنش‌های کیف پول نماینده
            if purge_wallet_txs:
                cursor.execute("UPDATE reseller_transactions SET is_deleted=1 WHERE reseller_id=?", (reseller_id,))

            # ۵. صفر کردن موجودی کارت‌های بانکی متصل به نماینده
            if reset_cards:
                cursor.execute("UPDATE reseller_cards SET balance = 0 WHERE reseller_id=?", (reseller_id,))
                try:
                    cursor.execute("""
                        UPDATE card_transactions
                        SET is_revoked=1, revoked_at=?, revoked_by=?, revoke_reason=?
                        WHERE owner_type='reseller' AND reseller_id=?
                    """, (now_iso, admin_name, f"ریست تراز توسط {admin_name}", reseller_id))
                except Exception:
                    pass

            # ۶. ثبت مارکر زمانی شروع دوره جدید
            cursor.execute("UPDATE resellers SET last_settled_at=?, settled_balance_marker=0, updated_at=? WHERE id=?", (now_iso, now_iso, reseller_id))

            # ۷. ثبت لاگ در تاریخچه و سیستم حسابداری
            desc_text = f"ریست کامل تراز و پاکسازی تراکنش‌های تستی نماینده «{r['name']}» (@{r['username']}) توسط {admin_name}"
            if notes:
                desc_text += f" - یادداشت: {notes}"

            cursor.execute("""
                INSERT INTO reseller_transactions (reseller_id, type, amount, balance_after, description, is_deleted, created_at)
                VALUES (?, 'settlement', 0, 0, ?, 0, ?)
            """, (reseller_id, desc_text, now_iso))

            try:
                cursor.execute("""
                    INSERT INTO accounting_records
                    (type, category, title, amount, source, ref_type, ref_id, description, date, created_at)
                    VALUES ('expense', 'settlement', ?, 0, 'reseller_reset', 'reseller', ?, ?, ?, ?)
                """, (f"ریست تراز مالی نماینده {r['name']}", str(reseller_id), desc_text, today_str, now_iso))
            except Exception:
                pass
            ...
            try:
                cursor.execute("""
                    INSERT INTO system_activity_logs
                    (category, action, title, description, actor_type, actor_id, actor_name, target_type, target_id, target_name, level, created_at)
                    VALUES ('financial', 'reset_finances', ?, ?, 'admin', ?, ?, 'reseller', ?, ?, 'warning', ?)
                """, (
                    f"ریست مالی نماینده {r['name']}",
                    desc_text,
                    admin_id,
                    admin_name,
                    reseller_id,
        ...
        try:
            ...
            if int_amt is not None and tx["status"] in ("approved", "completed"):
                ...
                if r_id and is_bundle and delta != 0:
                    r_cur = cursor.execute("SELECT balance FROM resellers WHERE id=?", (r_id,)).fetchone()
                    cur_r_bal = int(r_cur[0] or 0) if r_cur else 0
                    new_r_bal = cur_r_bal + delta
                    cursor.execute("UPDATE resellers SET balance = ?, updated_at=? WHERE id=?", (new_r_bal, now_iso, r_id))
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, '-', ?, ?)
                    """, (r_id, t_type, abs(delta), new_r_bal, tx.get("plan_name"), t_desc, now_iso))
    ...
    def migrate_add_columns(self):
        ...
        try:
            ...
            # تفکیک دسترسی به ربات اصلی و ربات فروش بسته
            if "bot_access_main" not in a_cols:
                cursor.execute("ALTER TABLE admin_users ADD COLUMN bot_access_main INTEGER DEFAULT 0")
                logger.info("Added bot_access_main to admin_users")
                if "bot_access" in a_cols:
                    cursor.execute("UPDATE admin_users SET bot_access_main = bot_access WHERE bot_access IS NOT NULL")
            if "bot_access_bundle" not in a_cols:
                cursor.execute("ALTER TABLE admin_users ADD COLUMN bot_access_bundle INTEGER DEFAULT 0")
                logger.info("Added bot_access_bundle to admin_users")
    ...
    def process_sub_reseller_affiliate_commission(self, sub_reseller_id: int, plan_price: int, plan_name: str, account_name: str, sub_id: int = None) -> dict:
        ...
        try:
            ...
            cursor.execute("SELECT balance FROM resellers WHERE id = ?", (parent_id,))
            p_row = cursor.fetchone()
            cur_p_bal = int(p_row[0] or 0) if p_row else 0
            new_p_bal = cur_p_bal + commission_amount
            cursor.execute("UPDATE resellers SET balance = ?, updated_at = ? WHERE id = ?", (new_p_bal, now, parent_id))

            # ۲. ثبت تراکنش در reseller_transactions
            cursor.execute("""
                INSERT INTO reseller_transactions (reseller_id, type, amount, balance_after, plan_name, account_name, description, created_at)
                VALUES (?, 'deposit', ?, ?, ?, ?, ?, ?)
            """, (parent_id, commission_amount, new_p_bal, plan_name, account_name, desc, now))
    ...
    def get_reseller_transactions(self, reseller_id: int, limit: int = None, year: int = None, month: int = None, day: int = None, date_str: str = None):
        """لیست تراکنش‌های یک نماینده به همراه صادرکننده، نوع فروش، حاشیه سود و مانده پس از تراکنش با پشتیبانی از فیلتر سال، ماه و روز جلالی"""
        import jdatetime
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        # اگر فیلتر ماه یا سال یا روز مشخص نشده باشد، لیمیت پیش‌فرض ۱۰۰ اعمال می‌شود
        ...
        for r in rows:
            ...
            dt_j = self._parse_jalali_datetime(created_at)

            # فیلتر ماه، سال و روز در صورت وجود
            if year and dt_j and dt_j.year != year:
                continue
            if month and dt_j and dt_j.month != month:
                continue
            if day and dt_j and dt_j.day != day:
                continue
            if date_str and dt_j and f"{dt_j.year}/{dt_j.month:02d}/{dt_j.day:02d}" != str(date_str).replace("-", "/"):
                continue

            if dt_j:
                tx["jalali_date"] = f"{dt_j.year}/{dt_j.month:02d}/{dt_j.day:02d}"
                tx["jalali_month"] = dt_j.month
                tx["jalali_year"] = dt_j.year
                tx["jalali_day"] = dt_j.day
                tx["jalali_month_name"] = jdatetime.date.j_months_fa[dt_j.month - 1] if 1 <= dt_j.month <= 12 else ""
                t_time = created_at[11:16] if len(created_at) >= 16 else "۰۰:۰۰"
                tx["jalali_time"] = t_time
    ...
    def get_reseller_monthly_transaction_counts(self, reseller_id: int, year: int) -> dict:
        """شمارش تعداد تراکنش‌های نماینده در هر یک از ۱۲ ماه سال جلالی مشخص"""
        conn = self.get_connection()
        cursor = conn.cursor()
        counts = {m: 0 for m in range(1, 13)}
        try:
            cursor.execute("SELECT created_at FROM reseller_transactions WHERE reseller_id = ?", (reseller_id,))
            for row in cursor.fetchall():
                dt_j = self._parse_jalali_datetime(row[0])
                if dt_j and dt_j.year == year and 1 <= dt_j.month <= 12:
                    counts[dt_j.month] += 1
        except Exception as e:
            logger.error(f"Error getting monthly tx counts: {e}")
        finally:
            conn.close()
        return counts

    def get_reseller_daily_transaction_counts(self, reseller_id: int, year: int, month: int) -> dict:
        """شمارش تعداد تراکنش‌های نماینده برای هر روز از ماه جلالی مشخص"""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        try:
            ...
            for row in cursor.fetchall():
                dt_j = self._parse_jalali_datetime(row[0])
                if dt_j and dt_j.year == year and dt_j.month == month and 1 <= dt_j.day <= 31:
                    counts[dt_j.day] += 1
        except Exception as e:
            logger.error(f"Error getting daily tx counts: {e}")
        finally:
            conn.close()
        return counts

    def get_admin_deposit_transactions(self, year: int = None, month: int = None, day: int = None, date_str: str = None) -> list:
        """
        واکشی جامع تمامی تراکنش‌های واریزی به مدیریت:
        - شامل واریزی‌ها به کارت‌های بانکی، ولت‌های کریپتو، درگاه‌ها و صندوق‌های نقدی
        - شامل فیش‌ها و سفارشات تاییدشده کاربران و نمایندگان (بسته‌ها و شارژها)
        - جلوگیری دقیق از ثبت رکوردهای تکراری از طریق رهگیری ref_id
        - غنی‌سازی با متادیتای تقویمی جلالی (سال، ماه، روز، ساعت) و عنوان منبع
        """
        import jdatetime
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        try:
            # ۱. تراکنش‌های واریزی ثبت شده در کارت‌ها و حساب‌های مدیریت
            cursor.execute("""
                SELECT ct.*, bc.bank_name, bc.card_holder, bc.card_number as b_card_number, bc.account_type as b_acc_type
                FROM card_transactions ct
                LEFT JOIN bank_cards bc ON ct.card_id = bc.id
                WHERE ct.owner_type = 'admin' AND ct.type = 'deposit' AND (ct.is_revoked = 0 OR ct.is_revoked IS NULL)
                ORDER BY ct.created_at DESC, ct.id DESC
            """)
            card_rows = cursor.fetchall()
            for r in card_rows:
                rd = dict(r)
                if rd.get("ref_id") and rd.get("ref_type") in ("transaction", "reseller_bundle", "reseller_receipt"):
                    ref_ids_seen.add(str(rd["ref_id"]))

                acc_name = f"{rd.get('bank_name') or ''} - {rd.get('card_holder') or ''}".strip(" -") or f"حساب #{rd.get('card_id')}"
                cat = rd.get("category") or "واریز به حساب"
                badge = "primary"
                if "مشتری" in cat:
                    badge = "success"
                elif "صندوق" in cat:
                ...
                items.append({
                    ...
                    "balance_after": int(rd.get("balance_after") or 0),
                    "account_name": acc_name,
                    "depositor": rd.get("created_by") or "سیستم / مدیریت",
                    "tracking_code": rd.get("tracking_code") or f"CT-{rd['id']}",
                    "description": rd.get("title") or rd.get("description") or "واریز به حساب مدیریت",
                    "created_at": rd.get("created_at") or ""
                })

            # ۲. سفارشات و فیش‌های پرداختی تاییدشده کاربران و نمایندگان (در صورت عدم ثبت در کارت‌ها)
            cursor.execute("""
                SELECT t.*, bc.bank_name, bc.card_holder
                FROM transactions t
                LEFT JOIN bank_cards bc ON (t.card_number = bc.card_number OR t.card_number LIKE '%' || SUBSTR(bc.card_number, -4))
                WHERE t.status IN ('approved', 'completed') AND (t.is_deleted = 0 OR t.is_deleted IS NULL)
                  AND ((t.reseller_id IS NULL OR t.reseller_id = 0) OR (t.gateway = 'bundle_reseller' OR t.order_id LIKE 'R_BUNDLE%' OR t.order_id LIKE 'R_TOPUP%'))
                ORDER BY t.created_at DESC, t.id DESC
            """)
            tx_rows = cursor.fetchall()
            for t in tx_rows:
                td = dict(t)
                if str(td["id"]) in ref_ids_seen:
                    continue

                gw = td.get("gateway") or ""
                oid = str(td.get("order_id") or "")
                pname = td.get("plan_name") or ""

                if gw == "card_to_card":
                    cat = "کارت به کارت"
                    badge = "info"
                elif gw in ("zarinpal", "blupal", "nextpay", "idpay"):
                    cat = f"درگاه {gw}"
                    badge = "warning"
                elif gw == "bundle_reseller" or oid.startswith("R_BUNDLE"):
                    cat = "بسته اعتباری نماینده"
                    badge = "primary"
                elif oid.startswith("R_TOPUP") or "شارژ" in pname:
                    cat = "شارژ کیف پول نماینده"
                    badge = "success"
                elif gw == "admin_manual":
                    cat = "واریز دستی مدیریت"
                    badge = "secondary"
                else:
                    cat = "واریز تایید شده"
                    badge = "success"

                acc_name = f"{td.get('bank_name') or ''} - {td.get('card_holder') or ''}".strip(" -") or (f"درگاه {gw}" if gw else "حساب اصلی")
                depositor = td.get("username") or (f"کاربر {td.get('user_id')}" if td.get("user_id") else f"نماینده #{td.get('reseller_id')}")
                ...
                items.append({
                    ...
                    "category_label": cat,
                    "category_badge": badge,
                    "amount": int(td.get("amount") or 0),
                    "balance_after": 0,
                    "account_name": acc_name,
                    "depositor": depositor,
                    "tracking_code": td.get("tracking_code") or td.get("order_id") or str(td["id"]),
                    "description": f"پرداخت {pname} ({oid})" if pname else oid,
                    "created_at": td.get("created_at") or ""
                })
        except Exception as e:
            logger.error(f"Error fetching admin deposit transactions: {e}")
        finally:
            conn.close()

        # فیلتر تقویمی جلالی و غنی‌سازی متادیتا
        filtered_items = []
        for it in items:
            dt_j = self._parse_jalali_datetime(it.get("created_at"))
            if dt_j:
                it["jalali_date"] = f"{dt_j.year}/{dt_j.month:02d}/{dt_j.day:02d}"
                it["jalali_month"] = dt_j.month
                it["jalali_year"] = dt_j.year
                it["jalali_day"] = dt_j.day
                it["jalali_month_name"] = jdatetime.date.j_months_fa[dt_j.month - 1] if 1 <= dt_j.month <= 12 else ""
                c_at = it.get("created_at") or ""
                it["jalali_time"] = c_at[11:16] if len(c_at) >= 16 else "۰۰:۰۰"

                if year and dt_j.year != year:
                    continue
                if month and dt_j.month != month:
                    continue
                if day and dt_j.day != day:
                    continue
                if date_str and it["jalali_date"] != str(date_str).replace("-", "/"):
                    continue
            else:
                if year or month or day or date_str:
                    continue
                it["jalali_date"] = "-"
                it["jalali_time"] = "-"
                it["jalali_month_name"] = ""

            filtered_items.append(it)

        filtered_items.sort(key=lambda x: x.get("created_at") or "", reverse=True)
        return filtered_items

    def get_admin_monthly_deposit_counts(self, year: int) -> dict:
        """شمارش تعداد واریزی‌های مدیریت در هر یک از ۱۲ ماه سال جلالی مشخص"""
        txs = self.get_admin_deposit_transactions()
        counts = {m: 0 for m in range(1, 13)}
        for t in txs:
            if t.get("jalali_year") == year and 1 <= t.get("jalali_month", 0) <= 12:
                counts[t["jalali_month"]] += 1
        return counts

    def get_admin_daily_deposit_counts(self, year: int, month: int) -> dict:
        """شمارش تعداد واریزی‌های مدیریت برای هر روز از ماه جلالی مشخص"""
        txs = self.get_admin_deposit_transactions(year=year, month=month)
        counts = {d: 0 for d in range(1, 32)}
        for t in txs:
            day_val = t.get("jalali_day")
            if day_val and 1 <= day_val <= 31:
                counts[day_val] += 1
        return counts

    @staticmethod
    def get_admin_deposit_stats(transactions: list) -> dict:
        """محاسبه آمار و ارقام جامع واریزی‌های مدیریت"""
        ...
        for t in transactions:
            amt = t.get("amount", 0)
            cat = str(t.get("category_label") or "")
            if "درگاه" in cat:
                gateway_amount += amt
            elif "صندوق" in cat or "نقدی" in cat:
                cash_amount += amt
            elif "نماینده" in cat or "بسته" in cat:
                reseller_amount += amt
            else:
                card_amount += amt
        ...
        return {
            ...
            "avg_amount": int(total_amount / total_count) if total_count > 0 else 0
        }
    ...
    def get_reseller_full_payment_history(self, reseller_id: int, period_filter: str = "all") -> dict:
        ...
        cursor.execute(f"""
            ...
            WHERE reseller_id = ? AND (is_deleted = 0 OR is_deleted IS NULL){date_cond_rtx}
            ORDER BY created_at DESC
        """, tuple(params_rtx))
        ...
        cursor.execute(f"""
            SELECT * FROM transactions 
            WHERE reseller_id = ? AND (is_deleted = 0 OR is_deleted IS NULL){date_cond_tx}
            ORDER BY created_at DESC
        """, tuple(params_tx))
        ...
        # ۳. محاسبات مالی دقیق (با حذف کامل تراکنش‌های باطل‌شده و حذف‌شده)
        cursor.execute(f"""
        ...
        cursor.execute(f"SELECT COUNT(*) FROM transactions WHERE reseller_id=? AND (is_deleted = 0 OR is_deleted IS NULL) AND (gateway='bundle_reseller' OR order_id LIKE 'R_BUNDLE%'){date_cond_tx}", tuple(params_tx))
    ...
    def cancel_queue_item(self, queue_id: int, reseller_id: int = None) -> dict:
        ...
        try:
            ...
            if r_id and cost > 0:
                ...
                r_row = cursor.execute("SELECT balance FROM resellers WHERE id=?", (r_id,)).fetchone()
                ...
                if renew_source == "credit":
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, 'refund_credit', ?, ?, ?, ?, ?, 'credit', ?, ?)
                    """, (r_id, cost, cur_r_bal, item_dict.get("plan_name", ""), account_name, "استرداد وجه لغو بسته رزرو در صف (کاهش بدهی اعتبار)", sub_id, now))
                else:
                    new_r_bal = cur_r_bal + cost
                    cursor.execute("UPDATE resellers SET balance = ?, updated_at=? WHERE id=?", (new_r_bal, now, r_id))
                    cursor.execute("""
                        INSERT INTO reseller_transactions (reseller_id, type, amount, balance_after, plan_name, account_name, description, payment_source, subscription_id, created_at)
                        VALUES (?, 'refund', ?, ?, ?, ?, ?, 'wallet', ?, ?)
                    """, (r_id, cost, new_r_bal, item_dict.get("plan_name", ""), account_name, "استرداد وجه لغو بسته رزرو در صف (واریز به کیف پول)", sub_id, now))
    ...
    def get_admin_manager_by_telegram_id(self, telegram_id: int) -> Optional[dict]:
        """دریافت مدیر یا کارمند پنل مدیریت کل بر اساس آیدی عددی تلگرام و تیک اجازه دسترسی ربات اصلی"""
        if not telegram_id:
            return None
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute("""
                ...
                WHERE telegram_id = ? AND (reseller_id IS NULL OR reseller_id = 0) AND is_active = 1 
    ...
    def get_admin_manager_by_telegram_id_for_bundle(self, telegram_id: int) -> Optional[dict]:
        """دریافت مدیر یا کارمند پنل مدیریت بر اساس آیدی عددی تلگرام و تیک اجازه دسترسی ربات فروش بسته"""
        if not telegram_id:
            return None
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute("""
                SELECT * FROM admin_users
                WHERE telegram_id = ? AND (reseller_id IS NULL OR reseller_id = 0) AND is_active = 1 
                  AND (bot_access_bundle = 1 OR role = 'super_admin')
                LIMIT 1
            """, (int(telegram_id) if str(telegram_id).isdigit() else 0,))
            row = cursor.fetchone()
            return dict(row) if row else None
        except Exception as e:
            logger.error(f"Error in get_admin_manager_by_telegram_id_for_bundle: {e}")
            return None
        finally:
            conn.close()

    def get_main_bot_admins(self) -> List[dict]:
        """دریافت لیست تمامی مدیران مجاز به استفاده و مدیریت در ربات اصلی مدیریت"""
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute("""
            ...
            logger.error(f"Error in get_main_bot_admins: {e}")
            return []
        finally:
            conn.close()

    def get_bundle_bot_admins(self) -> List[dict]:
        """دریافت لیست تمامی مدیران مجاز به استفاده و مدیریت در ربات فروش بسته نمایندگی"""
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute("""
            ...
            logger.error(f"Error in get_bundle_bot_admins: {e}")
            return []
        finally:
            conn.close()
    ...
    def get_active_plans(self, *args, **kwargs):
        """دریافت پلن‌های فعال از طریق admin_manager"""
        try:
            from admin_manager import get_active_plans
            return get_active_plans(*args, **kwargs)
        except Exception as e:
            logger.warning(f"Error calling get_active_plans: {e}")
            return {}

    def get_all_plans(self, *args, **kwargs):
        """دریافت تمامی پلن‌ها از طریق admin_manager"""
        try:
            from admin_manager import get_all_plans
            return get_all_plans(*args, **kwargs)
        except Exception as e:
            logger.warning(f"Error calling get_all_plans: {e}")
            return {}
    ...
    def get_accounting_partners_data(self) -> dict:
        ...
        try:
            ...
            for card in partner_cards:
                ...
                card_id = card['id']
                cursor.execute("SELECT COALESCE(SUM(amount), 0) FROM card_transactions WHERE card_id=? AND tx_type='deposit'", (card_id,))
    ...
    @staticmethod
    def _parse_jalali_datetime(date_val):
        """تبدیل ایمن انواع فرمت‌های تاریخ (ISO، استاندارد، جلالی متنی) به آبجکت jdatetime"""
        if not date_val:
            return None
        import jdatetime
        from datetime import datetime
        s = str(date_val).strip()
        clean_s = s[:10].replace('/', '-')
        parts = clean_s.split('-')
        if len(parts) == 3 and parts[0].isdigit() and parts[1].isdigit() and parts[2].isdigit():
            y, m, d = int(parts[0]), int(parts[1]), int(parts[2])
            if 1300 <= y <= 1500:
                return jdatetime.datetime(y, m, d)
            elif y > 1900:
                try:
                    dt_g = datetime(y, m, d)
                    return jdatetime.datetime.fromgregorian(datetime=dt_g)
                except Exception:
                    pass
        try:
            clean_iso = s.replace('T', ' ')[:19]
            dt_g = datetime.strptime(clean_iso, '%Y-%m-%d %H:%M:%S')
            return jdatetime.datetime.fromgregorian(datetime=dt_g)
        except Exception:
            pass
        try:
            clean_iso = s.replace('T', ' ')[:10]
            dt_g = datetime.strptime(clean_iso, '%Y-%m-%d')
            return jdatetime.datetime.fromgregorian(datetime=dt_g)
        except Exception:
            pass
        return None

    def get_reseller_partners(self, reseller_id: int) -> list:
        """واکشی لیست شرکای نماینده از جدول reseller_cards"""
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute("""
                SELECT * FROM reseller_cards 
                WHERE reseller_id = ? AND account_type = 'partner_savings'
                ORDER BY id ASC
            """, (reseller_id,))
            rows = cursor.fetchall()
            return [dict(r) for r in rows]
        except Exception as e:
            logger.error(f"Error in get_reseller_partners: {e}")
            return []
        finally:
            conn.close()

    def add_reseller_partner(self, reseller_id: int, name: str, percent: float,
                             card_number: str = None, bank_name: str = None,
                             assigned_to: str = None, notes: str = None) -> dict:
        """افزودن شریک جدید با درصد مشخص به سیستم حسابداری نماینده"""
        import time
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            if not card_number:
                card_number = f"R{reseller_id}-PRT-{int(time.time()) % 100000}"
            now = get_now_iso()
            cursor.execute("""
                INSERT INTO reseller_cards (
                    reseller_id, card_number, card_holder, bank_name,
                    balance, initial_balance, account_type, is_active,
                    profit_percent, assigned_to, notes, created_at
                ) VALUES (?, ?, ?, ?, 0, 0, 'partner_savings', 1, ?, ?, ?, ?)
            """, (reseller_id, card_number, name, bank_name or 'صندوق شراکت', float(percent or 0), assigned_to or name, notes or '', now))
            new_id = cursor.lastrowid
            conn.commit()
            return {"success": True, "partner_id": new_id}
        except Exception as e:
            logger.error(f"Error adding reseller partner: {e}")
            return {"success": False, "error": str(e)}
        finally:
            conn.close()

    def update_reseller_partner(self, partner_id: int, reseller_id: int, **kwargs) -> dict:
        """ویرایش مشخصات یا درصد سود شریک"""
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            allowed = ["card_holder", "profit_percent", "card_number", "bank_name", "assigned_to", "notes", "is_active"]
            updates = {k: v for k, v in kwargs.items() if k in allowed and v is not None}
            if not updates:
                return {"success": False, "error": "داده‌ای برای ویرایش ارسال نشده است."}
            fields = ", ".join([f"{k}=?" for k in updates.keys()])
            values = list(updates.values()) + [partner_id, reseller_id]
            cursor.execute(f"UPDATE reseller_cards SET {fields} WHERE id = ? AND reseller_id = ? AND account_type = 'partner_savings'", values)
            conn.commit()
            return {"success": True}
        except Exception as e:
            logger.error(f"Error updating reseller partner: {e}")
            return {"success": False, "error": str(e)}
        finally:
            conn.close()

    def delete_reseller_partner(self, partner_id: int, reseller_id: int) -> dict:
        """حذف شریک از سیستم نماینده"""
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute("DELETE FROM reseller_cards WHERE id = ? AND reseller_id = ? AND account_type = 'partner_savings'", (partner_id, reseller_id))
            conn.commit()
            return {"success": True}
        except Exception as e:
            logger.error(f"Error deleting reseller partner: {e}")
            return {"success": False, "error": str(e)}
        finally:
            conn.close()

    def settle_reseller_partner(self, partner_id: int, reseller_id: int, amount: int, description: str = None) -> dict:
        """ثبت تسویه‌حساب یا واریزی به شریک"""
        import jdatetime
        conn = self.get_connection()
        cursor = conn.cursor()
        now = get_now_iso()
        ...
        try:
            cursor.execute("SELECT * FROM reseller_cards WHERE id = ? AND reseller_id = ? AND account_type = 'partner_savings'", (partner_id, reseller_id))
            p_card = cursor.fetchone()
            if not p_card:
                return {"success": False, "error": "حساب شریک یافت نشد."}
            p_dict = dict(p_card)
            partner_name = p_dict.get("card_holder") or p_dict.get("assigned_to") or "شریک"

            # ثبت در card_transactions با رعایت قید category
            cursor.execute("""
                INSERT INTO card_transactions (card_id, amount, type, category, title, description, created_at, owner_type, reseller_id)
                VALUES (?, ?, 'deposit', 'تسویه شرکا', ?, ?, ?, 'reseller', ?)
            """, (partner_id, amount, f"تسویه سود با {partner_name}", description or "تسویه حساب سهم شراکت", now, reseller_id))

            # افزایش موجودی پرداختی به شریک
            cursor.execute("UPDATE reseller_cards SET balance = balance + ? WHERE id = ?", (amount, partner_id))

            # ثبت در رکوردهای حسابداری
            cursor.execute("""
                INSERT INTO accounting_records (
                    type, category, title, amount, source, description, date, created_at, reseller_id, card_name
                ) VALUES ('settlement', 'تسویه‌حساب شرکا', ?, ?, 'reseller_partner_settle', ?, ?, ?, ?, ?)
            """, (f"تسویه سهم سود {partner_name}", amount, description or f"تسویه سهم سود شریک {partner_name}", now_shamsi, now, reseller_id, partner_name))

            conn.commit()
            return {"success": True}
        except Exception as e:
            logger.error(f"Error in settle_reseller_partner: {e}")
            return {"success": False, "error": str(e)}
        finally:
            conn.close()

    def get_reseller_users_for_partner_picker(self, reseller_id: int) -> list:
        """دریافت لیست کاربران نماینده جهت انتخاب داینامیک در فرم افزودن شریک"""
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            users_map = {}
            cursor.execute("SELECT id, telegram_id, username FROM users WHERE reseller_id = ?", (reseller_id,))
            for r in cursor.fetchall():
                u = dict(r)
                tid = u.get("telegram_id")
                uname = u.get("username")
                label = f"{uname} (@{tid})" if uname else f"کاربر {tid}"
                users_map[str(tid)] = {"id": u["id"], "telegram_id": tid, "username": uname, "display_name": label}

            cursor.execute("SELECT id, telegram_id, account_name FROM subscriptions WHERE reseller_id = ?", (reseller_id,))
            for r in cursor.fetchall():
                s = dict(r)
                tid = s.get("telegram_id")
                acc = s.get("account_name")
                key = str(tid or acc)
                if key not in users_map and (acc or tid):
                    label = f"{acc} ({tid})" if (acc and tid) else (acc or f"کاربر {tid}")
                    users_map[key] = {"id": s["id"], "telegram_id": tid, "username": acc, "display_name": label}

            return list(users_map.values())
        except Exception as e:
            logger.error(f"Error fetching reseller users for partner picker: {e}")
            return []
        finally:
            conn.close()

    def get_reseller_accounting_summary(self, reseller_id: int, year: int = None) -> dict:
        """خلاصه شاخص‌های مالی، سهم تفکیکی خود نماینده و شرکا و کارت‌های بانکی"""
        import jdatetime
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            ...
            # ۱. واکشی حساب‌ها و کارت‌های بانکی نماینده
            cursor.execute("SELECT * FROM reseller_cards WHERE reseller_id = ? ORDER BY is_default DESC, id ASC", (reseller_id,))
            all_cards = [dict(c) for c in cursor.fetchall()]

            partner_cards = [c for c in all_cards if c.get("account_type") == "partner_savings"]
            bank_accounts = [c for c in all_cards if c.get("account_type") != "partner_savings"]

            savings_balance = sum(int(c.get("balance") or 0) for c in partner_cards)
            total_cards_balance = sum(int(c.get("balance") or 0) for c in bank_accounts)

            for card in bank_accounts:
                c_id = card["id"]
                cursor.execute("SELECT COALESCE(SUM(amount), 0) FROM card_transactions WHERE card_id = ? AND type = 'deposit'", (c_id,))
                card["total_deposits"] = cursor.fetchone()[0] or 0
                cursor.execute("SELECT COALESCE(SUM(amount), 0) FROM card_transactions WHERE card_id = ? AND type = 'withdrawal'", (c_id,))
                card["total_withdrawals"] = cursor.fetchone()[0] or 0

            # ۲. درآمد حاصل از پرداخت‌های تایید شده مشتریان در transactions
            cursor.execute("""
                SELECT 
                    COALESCE(SUM(amount), 0) as total_approved_tx,
                    COUNT(*) as count_approved_tx
                FROM transactions 
                WHERE reseller_id = ? AND status IN ('approved', 'completed')
                  AND (is_deleted = 0 OR is_deleted IS NULL)
                  AND (gateway != 'bundle_reseller' AND order_id NOT LIKE 'R_BUNDLE%')
            ...
            total_approved_tx = tx_row["total_approved_tx"] or 0

            # ۳. خریدهای عمده و فروش‌ها در reseller_transactions
            cursor.execute("""
                SELECT 
            ...
            total_sales_rtx = sales_row["total_sales"] or 0
            total_cost_rtx = sales_row["total_cost"] or 0

            total_sales = max(total_approved_tx, total_sales_rtx)
            if total_sales == 0 and total_cost_rtx > 0:
                disc = r_dict.get("discount_percent") or 20
                total_sales = int(total_cost_rtx / (1 - (disc / 100)))

            total_cost = total_cost_rtx

            # ۴. درآمدهای دستی و هزینه‌های دستی
            ...
            # ۵. محاسبه سهم شرکا و سهم خود نماینده (مالک)
            partners_data = []
            total_partner_percent = 0
            total_partner_shares = 0
            total_partner_paid = 0
            ...
            for p in partner_cards:
                ...
                cursor.execute("""
                    SELECT COALESCE(SUM(amount), 0) FROM card_transactions 
                    WHERE card_id = ? AND type IN ('deposit', 'settlement')
                """, (p["id"],))
                paid_from_card = cursor.fetchone()[0] or 0
                paid_balance = max(int(p.get("balance") or 0), paid_from_card)
                total_partner_paid += paid_balance

                partners_data.append({
                    "id": p["id"],
                    "name": p_name,
                    "assigned_to": p.get("assigned_to") or p_name,
                    "percent": pct,
            ...
            reseller_profit_share = int(net_profit * reseller_share_percent / 100) if net_profit > 0 else 0
            ...
            reseller_paid_share = settled_debts + total_deposits
            reseller_remaining_share = max(0, reseller_profit_share - reseller_paid_share)
            ...
            standard_categories = [
                ...
                "هزینه‌های جاری و متفرقه",
                "واریز به صندوق پس‌انداز",
                "تسویه‌حساب شرکا"
            ]
            cursor.execute("SELECT DISTINCT category FROM accounting_records WHERE reseller_id = ? AND category IS NOT NULL", (reseller_id,))
            custom_cats = [row[0] for row in cursor.fetchall() if row[0] and row[0] not in standard_categories]
            all_categories = standard_categories + custom_cats
            ...
            return {
                ...
                "net_profit": net_profit,
                "reseller_share_percent": reseller_share_percent,
                "reseller_profit_share": reseller_profit_share,
                "reseller_paid_share": reseller_paid_share,
                "reseller_remaining_share": reseller_remaining_share,
                "partners": partners_data,
                "total_partner_percent": total_partner_percent,
                "total_partner_shares": total_partner_shares,
                "total_partner_paid": total_partner_paid,
    ...
    def get_reseller_jalali_monthly_accounting(self, reseller_id: int, year: int = None) -> list:
        """محاسبه دقیق درآمد، هزینه و سود ماهانه ۱۲ ماه سال جلالی با پارس استاندارد تاریخ"""
        import jdatetime
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            ...
            # ۱. تراکنش‌های خرید و تمدید اشتراک در reseller_transactions
            cursor.execute("""
                SELECT selling_price, amount, profit_margin, created_at, type 
                FROM reseller_transactions 
                WHERE reseller_id = ? AND type IN ('purchase', 'purchase_credit', 'renewal', 'renew', 'renew_credit')
                  AND (is_revoked = 0 OR is_revoked IS NULL) AND (status != 'revoked' OR status IS NULL)
                  AND (is_deleted = 0 OR is_deleted IS NULL)
            """, (reseller_id,))
            for row in cursor.fetchall():
                dt_j = self._parse_jalali_datetime(row["created_at"])
                if not dt_j or dt_j.year != year:
                    continue
                m_idx = dt_j.month - 1
                s_price = int(row["selling_price"] or 0)
                cost = int(row["amount"] or 0)
                if s_price == 0 and cost > 0:
                    s_price = int(cost * 1.3)
            ...
            # ۲. پرداخت‌های مستقیم مشتریان در transactions
            cursor.execute("""
                SELECT id, order_id, amount, created_at, is_renewal 
                FROM transactions 
                WHERE reseller_id = ? AND status IN ('approved', 'completed')
                  AND (is_deleted = 0 OR is_deleted IS NULL)
                  AND (gateway != 'bundle_reseller' AND order_id NOT LIKE 'R_BUNDLE%')
            ...
            for row in cursor.fetchall():
                ...
                if months[m_idx]["income"] == 0:
                    months[m_idx]["income"] += amt
                    est_cost = int(amt * 0.7)
                    months[m_idx]["expense"] += est_cost
                    months[m_idx]["profit"] += (amt - est_cost)

            # ۳. درآمدهای دستی در accounting_records
            cursor.execute("SELECT amount, date, created_at FROM accounting_records WHERE reseller_id = ? AND type = 'income'", (reseller_id,))
            for row in cursor.fetchall():
                d_str = row["date"] or row["created_at"]
                dt_j = self._parse_jalali_datetime(d_str)
            ...
            # ۴. هزینه‌های دستی در accounting_records
    ...
    def get_reseller_accounting_transactions(self, reseller_id: int, year: int = None, month: int = None) -> list:
        """واکشی لیست جامع تراکنش‌ها با تمام متادیتاها (شماره سفارش، مشتری، پلن، سود، پیگیری و دسته‌بندی عملیاتی)"""
        import jdatetime
        conn = self.get_connection()
        cursor = conn.cursor()
        items = []
        try:
            ...
            cursor.execute("SELECT id, telegram_id, account_name, plan_name, data_limit, duration, phone_number FROM subscriptions WHERE reseller_id = ?", (reseller_id,))
            sub_map = {}
            for s in cursor.fetchall():
                sub_dict = dict(s)
                sub_map[sub_dict["id"]] = sub_dict
                if sub_dict.get("account_name"):
                    sub_map[sub_dict["account_name"]] = sub_dict

            seen_order_keys = set()

            # ۱. تراکنش‌های جدول transactions (پرداخت‌های تایید شده مشتریان)
            cursor.execute("""
                SELECT * FROM transactions 
                WHERE reseller_id = ? AND (gateway != 'bundle_reseller' AND order_id NOT LIKE 'R_BUNDLE%')
                  AND (is_deleted = 0 OR is_deleted IS NULL)
                ORDER BY id DESC LIMIT 500
            """, (reseller_id,))
            for r in cursor.fetchall():
                r_dict = dict(r)
                dt_j = self._parse_jalali_datetime(r_dict.get("created_at"))
                if not dt_j or dt_j.year != year:
                    continue

                order_id = r_dict.get("order_id") or f"ORD-{r_dict['id']}"
                seen_order_keys.add(order_id)
                status = r_dict.get("status") or "approved"
                is_renewal = bool(r_dict.get("is_renewal"))
                
                category = "تمدید اشتراک" if is_renewal else "فروش اشتراک جدید"
                badge_type = "primary" if is_renewal else "customer"

                matched_sub = sub_map.get(r_dict.get("subscription_id")) or sub_map.get(r_dict.get("account_name"))
                customer_name = r_dict.get("account_name") or r_dict.get("username")
                if not customer_name and matched_sub:
                    customer_name = matched_sub.get("account_name")
                if not customer_name:
                    customer_name = f"کاربر {r_dict.get('user_id') or r_dict['id']}"

                plan_name = r_dict.get("plan_name")
                if not plan_name and matched_sub:
                    plan_name = matched_sub.get("plan_name")
                plan_name = plan_name or "اشتراک اختصاصی"

                amount = int(r_dict.get("amount") or 0)
                cost = int(amount * 0.7)
                profit = max(0, amount - cost)

                t_time = str(r_dict.get("created_at") or "")[11:16] or "۰۰:۰۰"
                items.append({
                    "id": r_dict["id"],
                    "order_id": order_id,
                    "source": "transaction",
                    ...
                    "type_label": category,
                    "category": category,
                    "badge_color": "info" if not is_renewal else "primary",
                    "amount": amount,
                    "cost_amount": cost,
                    "profit_margin": profit,
                    "selling_price": amount,
                    "title": f"{category} - {plan_name}",
                    "customer_name": customer_name,
                    "customer_id": r_dict.get("user_id") or "",
                    "plan_name": plan_name,
                    "gateway": r_dict.get("gateway") or "کارت به کارت",
                    "tracking_code": r_dict.get("tracking_code") or r_dict.get("ref_id") or "---",
                    "card_number": r_dict.get("card_number") or "",
                    "bank_name": r_dict.get("gateway") or "درگاه بانکی",
                    "status": status,
                    "status_label": "موفق / تایید شده" if status in ('approved', 'completed') else ("در انتظار" if status == 'pending' else "ناموفق"),
                    "description": r_dict.get("account_comment") or f"پرداخت سفارش {order_id}",
                    ...
                    "timestamp": str(r_dict.get("created_at") or ""),
                    "can_edit": False
                })

            # ۲. خریدهای ثبت شده در reseller_transactions
            cursor.execute("""
                SELECT * FROM reseller_transactions 
                WHERE reseller_id = ? AND (is_deleted = 0 OR is_deleted IS NULL)
                  AND (is_revoked = 0 OR is_revoked IS NULL) AND (status != 'revoked' OR status IS NULL)
            ...
            for r in cursor.fetchall():
                ...
                rtx_type = r_dict.get("type", "purchase")
                sub_id = r_dict.get("subscription_id")
                acc_name = r_dict.get("account_name")
                synth_order_id = f"SUB-{sub_id or r_dict['id']}"

                if synth_order_id in seen_order_keys:
                    continue

                if rtx_type in ('purchase', 'purchase_credit'):
                    cat = "فروش اشتراک جدید"
                    badge_type = "customer"
                    badge_color = "info"
                elif rtx_type in ('renewal', 'renew', 'renew_credit'):
                    cat = "تمدید اشتراک"
                    badge_type = "customer"
                    badge_color = "primary"
                elif rtx_type == 'deposit':
                    cat = "شارژ کیف پول / افزایش اعتبار"
                    badge_type = "customer"
                    badge_color = "success"
                else:
                    cat = "تراکنش مالی"
                ...
                cost = int(r_dict.get("amount") or 0)
                selling = int(r_dict.get("selling_price") or (cost * 1.3 if cost > 0 else 0))
                profit = int(r_dict.get("profit_margin") or max(0, selling - cost))

                t_time = str(r_dict.get("created_at") or "")[11:16] or "۰۰:۰۰"
                matched_sub = sub_map.get(sub_id) or sub_map.get(acc_name)
                p_name = r_dict.get("plan_name") or (matched_sub.get("plan_name") if matched_sub else "اشتراک اختصاصی")
                ...
                items.append({
                    ...
                    "amount": selling if selling > 0 else cost,
                    "cost_amount": cost,
                    "profit_margin": profit,
                    "selling_price": selling,
                    "title": f"{cat} - {p_name}",
                    "customer_name": acc_name or "مشتری ربات",
                    "customer_id": matched_sub.get("telegram_id") if matched_sub else "",
                    "plan_name": p_name,
                    "gateway": "کیف پول نماینده / اعتباری",
                    "tracking_code": f"RTX-{r_dict['id']}",
                    "card_number": "",
                    "bank_name": "کیف پول پنل",
                    "status": "completed",
                    "status_label": "تکمیل شده",
                    "description": r_dict.get("description") or f"{cat} برای {acc_name}",
                    "jalali_date": f"{dt_j.day} {jdatetime.date.j_months_fa[dt_j.month - 1]} {dt_j.year}",
                    "jalali_time": f"ساعت {t_time}",
                    "jalali_month": dt_j.month,
                    "jalali_year": dt_j.year,
            ...
            # ۳. رکوردهای دستی در accounting_records
            cursor.execute("SELECT * FROM accounting_records WHERE reseller_id = ? ORDER BY id DESC", (reseller_id,))
            for r in cursor.fetchall():
                r_dict = dict(r)
                dt_j = self._parse_jalali_datetime(r_dict.get("date") or r_dict.get("created_at"))
                if not dt_j or dt_j.year != year:
                    continue

                rtype = r_dict.get("type", "expense")
                cat = r_dict.get("category") or ("هزینه‌های جاری و متفرقه" if rtype == 'expense' else "دریافتی از مشتری")
                if rtype == 'savings':
                    t_cat = "savings"
                    b_color = "purple"
                    cat = "واریز به صندوق پس‌انداز"
                ...
                t_time = str(r_dict.get("created_at") or "")[11:16] or "۱۲:۰۰"
                items.append({
                    "id": r_dict["id"],
                    "order_id": f"REC-{r_dict['id']}",
                    "source": "manual",
                    "type": t_cat,
                    "type_label": cat,
                    "category": cat,
                    "badge_color": b_color,
                    "amount": int(r_dict["amount"] or 0),
                    "cost_amount": int(r_dict["amount"] or 0) if rtype == 'expense' else 0,
                    ...
                    "jalali_month": dt_j.month,
                    "jalali_year": dt_j.year,
                    "timestamp": str(r_dict.get("created_at") or r_dict.get("date") or ""),
                    "can_edit": True
                })
    ...
    def get_accounting_summary(self) -> dict:
        ...
        try:
            ...
            # درآمدهای شرکا (فروش‌های ثبت شده تحت عنوان شریک)
            cursor.execute("SELECT COALESCE(SUM(selling_price), 0) FROM reseller_transactions WHERE type='purchase' AND payment_source='partner'")
            partner_income = cursor.fetchone()[0] or 0
    ...
    def create_admin_user(self, username: str, password: str, display_name: str,
                          ...
                          bot_access: int = 0, bot_access_main: int = None, bot_access_bundle: int = 0) -> dict:
        """افزودن مدیر جدید با نقش و دسترسی‌های مشخص، آیدی تلگرام، شماره تماس، درصد شراکت و دسترسی ربات‌ها"""
        conn = self.get_connection()
        cursor = conn.cursor()
        now = get_now_iso()
        password_hash = self.hash_password(password)
        effective_bot_main = 1 if (bot_access_main if bot_access_main is not None else bot_access) else 0
        effective_bot_bundle = 1 if bot_access_bundle else 0
        try:
            cursor.execute("""
                INSERT INTO admin_users (username, password_hash, display_name, role, permissions, is_active, created_at, telegram_id, phone, share_percent, debt_balance, bot_access, bot_access_main, bot_access_bundle)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?)
            """, (username.strip(), password_hash, display_name.strip(), role, permissions, 1 if is_active else 0, now, telegram_id, phone.strip() if phone else None, int(share_percent or 0), effective_bot_main, effective_bot_main, effective_bot_bundle))
    ...
    def update_admin_user(self, admin_id: int, **kwargs) -> dict:
        """ویرایش اطلاعات، نقش، دسترسی‌ها، درصد شراکت و دسترسی تفکیک‌شده ربات‌های یک مدیر"""
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            fields = []
            params = []
            if "bot_access_main" in kwargs and "bot_access" not in kwargs:
                kwargs["bot_access"] = kwargs["bot_access_main"]

            for key, val in kwargs.items():
                if key == "password" and val:
                    fields.append("password_hash=?")
                    params.append(self.hash_password(val))
                elif key in ["username", "display_name", "role", "permissions", "is_active", "telegram_id", "phone", "custom_avatar", "share_percent", "debt_balance", "bot_access", "bot_access_main", "bot_access_bundle", "logout_notification_enabled"]:
                    fields.append(f"{key}=?")
                    params.append(val)
    ...
    def apply_reseller_bundle_purchase(self, reseller_id: int, bundle_id: str) -> dict:
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO reseller_transactions (reseller_id, type, amount, balance_after, plan_name, account_name, description, created_at)
                VALUES (?, 'deposit', ?, ?, ?, '-', ?, ?)
            """, (reseller_id, credit_to_add, new_balance, bundle["title"], desc, now))
    ...
    def apply_reseller_bundle_credit(self, reseller_id: int, amount: int, bundle_title: str, tx_id: int = None) -> dict:
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO reseller_transactions (reseller_id, type, amount, balance_after, plan_name, account_name, description, created_at)
                VALUES (?, 'deposit', ?, ?, ?, '-', ?, ?)
            """, (reseller_id, credit_to_add, new_balance, bundle_title, desc, now))
